...
def simplify_target(data):
    ...
    for i in main_causes_of_accidents:
        ...
        if 200 < i < 210:
            convert_values.append(2)
        if 300 < i < 312:
            convert_values.append(3)
        if 400 < i < 415:
            convert_values.append(4)
        if 500 < i < 517:
            convert_values.append(5)
        if 600 < i < 616:
            convert_values.append(6)

    data["Hlavní_příčiny_nehody"] = array(convert_values)
    return data
